...
let basePath = "";
setTimeout(() => {
  basePath = window.ui.getConfigs().spec.servers[0]
    ? window.ui.getConfigs().spec.servers[0].url
    : "";
}, 500);
...
(function (exports) {
  ...
  async function connect() {
    if (typeof window.ethereum !== "undefined") {
      ...
      const signature = await etherumWallet.signatureMessage(
        responseNonce.address,
        message + responseNonce.nonce,
      );
      const responseSign = {
      ...
      document
        .querySelector(".auth-container .btn.modal-btn.auth.button")
        ?.click();
      document
        .querySelector(".auth-container .btn.modal-btn.auth.btn-done.button")
        ?.click();
      document.querySelector(".auth-wrapper .btn.authorize.unlocked").click();
      var input = document.querySelector(
        ".auth-container input[aria-label='auth-bearer-value']",
      );
      setNativeValue(input, loginRes?.tokens?.access?.token);

      document
        .querySelector(".auth-container .btn.modal-btn.auth.authorize.button")
        .click();
      document
        .querySelector(".auth-container .btn.modal-btn.auth.btn-done.button")
        .click();
